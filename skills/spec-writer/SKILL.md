...
> greenfield or in plain conversation, no project required. **When present**, it reads
> `.council/council.config.md` (the project's detected stack + gates, written by `council-init`) and
> `conventions.md` (accepted patterns from prior council reviews) to fit the spec to the project — a spec
> never proposes against a settled convention. These files only *inform*; their absence changes nothing,
> and the spec stays implementation-free either way.
...
5. **Check for steering files** — Look for `.council/council.config.md`, `conventions.md`, `.claude/`, `CLAUDE.md`, `constitution.md`, `CONVENTIONS.md`. These contain project-level rules and accepted patterns the spec must respect. The first two are the Ultra Council's own — when present, use their detected stack and settled decisions; when absent, carry on unchanged.
