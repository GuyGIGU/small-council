#!/usr/bin/env bash
# Scaffold: the propose-risky-change fixture — a small Python web app with a council; the feature branch
# changes the auth check (a missing token now passes), adds a schema migration and a test. Its UI
# templates are untouched. Self-contained: writes every file itself.
set -euo pipefail
git init -q
