...
2. **Blind verification.** Dispatch `small-council:council-verifier`:
   - Each **P1**, and each item on a **protected subject** — authorization, data loss, injection,
     ...
     batches of up to 8.
   - A verifier gets only the item's number, a one-sentence claim and its `path:line` — never the
     seat, principle, reasoning or fix — plus the brief's path and the code root. It must trace a
     live path through the code.
   - Include every cut P1: a lone dissenter may be right.
   - Each verifier writes its own file, `<run>/verify-<n>.md`. Track them like workers
